...
type Article_xiazhaoxuan struct {
	gorm.Model
...
// 文章、对应文章分类信息
type ArticleRecord_xiazhaoxuan struct {
	Article_xiazhaoxuan
}

// 展示文章详情数据
type ArticleShow_xiazhaoxuan struct {
	Article_xiazhaoxuan
	UserInfo_xiazhaoxuan
}

// 发布文章
func (article *Article_xiazhaoxuan) CreateBlog() error {
	return DB.FirstOrCreate(&article, "title = ? or content = ?", article.Title, article.Content).Error
}

// 获取指定文章信息
func (article *Article_xiazhaoxuan) ShowArticleInfo() error {
	return DB.Where("id = ?", article.ID).Find(&article).Error
}

// 查询置顶的文章列表
func GetTopArticleList(nums int) (list []*Article_xiazhaoxuan, err error) {
...
// 查询博客列表 分页
func GetArticleList(UserId, page int) ([]*Article_xiazhaoxuan, int, error) {
	db := DB
	var (
		list      []*Article_xiazhaoxuan
		totalNums int
	)
	if UserId > 0 {
...
// 文章浏览量+1
func (article *Article_xiazhaoxuan) AddArticleView(id int) error {
	return DB.Model(&article).Update("view", gorm.Expr("view + ?", 1)).Error
}

// 删除文章
func (article *Article_xiazhaoxuan) DeleteArticle() error {
